...
class CacheManager {
    constructor(config = {}) {
        ...
        this.storageKey = config.storageKey || "kiemhang_persistent_cache";
        ...
        try {
            ...
            if (error.name === "QuotaExceededError") {
                ...
                try {
                    ...
                    console.error(
                        "Failed to save cache after cleanup:",
                        retryError,
                    );
                }
            }
...
const cacheManager = new CacheManager({
    ...
    storageKey: "kiemhang_persistent_cache",
});
...
function getCachedData() {
    // Try new persistent cache manager first
    ...
    // Fallback to old memory cache for backward compatibility
    if (memoryCache.data && memoryCache.timestamp) {
        if (Date.now() - memoryCache.timestamp < APP_CONFIG.CACHE_EXPIRY) {
            console.log("Using memory cached inventory data (fallback)");
...
window.debugCache = function () {
    ...
    console.log("Cache entries:", cacheManager.cache.size);

    // Show sample of cached data
    if (memoryCache.data && memoryCache.data.length > 0) {
        console.log("Sample data:", memoryCache.data[0]);
    }
